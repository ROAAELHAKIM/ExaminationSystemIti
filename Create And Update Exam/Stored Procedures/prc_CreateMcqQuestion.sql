CREATE OR ALTER PROCEDURE prc_CreateMCQQuestion
    @UserId INT,
    @CourseId INT,
...
BEGIN
    ...
    BEGIN
        ...
        VALUES (@QuestionText, 'MCQ', @DifficultyLevel, @CourseId, @DefaultMark);

        DECLARE @QuestionId INT = SCOPE_IDENTITY();

        -- ????? ?????????? ?? ????? ????
        INSERT INTO MCQAnswer (QuestionId, AnswerText, IsTrue)
        VALUES
            (@QuestionId, @A, CASE WHEN @IsTrue = 'A' THEN 1 ELSE 0 END),
            (@QuestionId, @B, CASE WHEN @IsTrue = 'B' THEN 1 ELSE 0 END),
            (@QuestionId, @C, CASE WHEN @IsTrue = 'C' THEN 1 ELSE 0 END),
            (@QuestionId, @D, CASE WHEN @IsTrue = 'D' THEN 1 ELSE 0 END);
