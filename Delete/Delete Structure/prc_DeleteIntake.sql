CREATE OR ALTER PROCEDURE prc_DeleteIntake
    @user_id INT,
    @branch_id INT,
    @intake_id INT
AS
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            RAISERROR('Access denied. Only Training Managers or Admins can delete intakes.', 16, 1);
            RETURN;
        END
        ...
        IF NOT EXISTS (
            SELECT 1
            FROM Intake
            WHERE BranchId = @branch_id AND IntakeId = @intake_id
        )
        BEGIN
...
EXEC prc_DeleteIntake @user_id = 15, @branch_id = 7, @intake_id = 10;